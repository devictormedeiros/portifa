...
const SectionHeader = ({data}) => {
    ...
    return (
        <section className="flex flex-col w-full items-center md:gap-10 gap-8 px-6 py-0 relative container relative mt-[-3rem]">
            ...
            <div className="lg:min-w-[41.125rem]">
                ...
                <div className="flex items-center gap-[2.0625rem] mb-[1rem] lg:gap-[4rem]">
                    ...
                    <a href={data?.acf?.link?.url} title="Acessar Projeto" className="text-gray-200 bg-white-10 hover:bg-primary menu-section flex items-center py-2 px-4 rounded-3xl gap-x-2 flex-none duration-500" target="_blank">
                ...
                {
                    data?.tecnologia_detalhes?.length > 0 && (
                        <div 
                        ref={scrollRef}
                        className="scroll-drag flex items-center gap-8 relative self-stretch w-full overflow-x-hidden list-categories w-full lg:max-w-[41.125rem] lg:gap-4">
                            {
                                data?.tecnologia_detalhes?.map((tecnologia, i) => (
                                    <button
                                        key={i}
                                        className="border border-white-20 text-white-70 hover:bg-primary hover:border-primary menu-section flex items-center py-[0.375rem] px-4 rounded-3xl gap-x-2 flex-none duration-500"
                                    >
                                        <img className="max-w-[1.0625rem] w-full h-auto" src={tecnologia?.acf?.tecnologias?.icone?.url} alt={tecnologia?.acf?.tecnologias?.icone?.alt}/>
                                        {tecnologia.name}
                                    </button>
                                ))
                            }
            ...
            <div className="text-white-70 feed-excerpt lg:flex-1"   dangerouslySetInnerHTML={{ __html: data?.excerpt?.rendered }} />
            </div>
        </section>
    )
}
