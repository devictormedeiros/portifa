        ...
        <article className="sticky top-[7.5rem] card-projeto col-span-12 bg-gradient-primary-c rounded-2xl">
            <div className="p-[2rem] flex flex-col gap-6 lg:gap-[2.5rem] rounded-lg lg:p-[4rem]">
                <div className="flex flex-col gap-[0.5rem] justify-between pb-[.5rem] border-b border-white-10 lg:flex-row">
                    ...
                    <div className="flex items-center gap-[1.25rem] lg:gap-6">
                    {projeto.technologies.map((tech, index) => (
                        <div className="flex cursor-pointer" key={index}>
                            <Image  className="max-w-[1rem] w-full h-full lg:max-w-6" src={`/icons/${tech}.svg`} alt="Tecnologia X" width={25} height={25}/>
                        </div>
                    ))}
                    </div>
                </div>

                <div className="flex flex-col gap-6 lg:gap-[2.5rem] lg:flex-row lg:items-center">
